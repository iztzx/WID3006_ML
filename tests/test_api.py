...
def test_valid_cohort_all_views():
    for view in ["aggregated", "category", "individual"]:
        response = client.get(f"/v1/cohorts?view={view}&metric=app_usage_time_min&category=gender")
        assert response.status_code == 200
        payload = response.json()
...
def test_prediction_alias():
    ...
    response = client.post("/api/predict", json={"app_usage_time_min": 100, "swipe_right_ratio": 0.3})
    assert response.status_code == 200
...
def test_batch_prediction():
    response = client.post("/v1/predict/batch", json={
        "scenarios": [
            {"app_usage_time_min": 100, "swipe_right_ratio": 0.5, "likes_received": 50,
             "mutual_matches": 10, "message_sent_count": 30, "bio_length": 140,
             "emoji_usage_rate": 0.3, "height_cm": 170, "weight_kg": 70,
             "profile_pics_count": 3, "last_active_hour": 12},
            {"app_usage_time_min": 50, "swipe_right_ratio": 0.8, "likes_received": 100,
             "mutual_matches": 20, "message_sent_count": 60, "bio_length": 200,
             "emoji_usage_rate": 0.6, "height_cm": 165, "weight_kg": 60,
             "profile_pics_count": 5, "last_active_hour": 18},
        ]
    })
    assert response.status_code == 200
    payload = response.json()
...
def test_data_service_can_use_normal_dataset_without_bmi(monkeypatch, tmp_path):
    ...
    for index in range(10):
        rows.append({
            ...
            "relationship_intent": "Serious Relationship" if index < 5 else "Friendship",
        })
...
def test_health_degraded_on_missing_artifacts(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ModelService, "required_artifacts",
