...
def construct_target(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df[TARGET_COL] = pd.qcut(
        df["engagement_score"], q=3, labels=[0, 1, 2]
    ).astype(int)
...
def scale_split_select(
...
) -> tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray, StandardScaler, LabelEncoder, list[str]]:
    ...
    sns.barplot(
        ...
        data=importance_df.head(20), palette="magma",
    ...
    return (
        ...
        y_train, y_test,
        scaler, target_encoder, selected_features,
    )
...
def main() -> None:
    ...
    pd.DataFrame(y_train, columns=["engagement"]).to_csv(OUTPUT_DIR / "y_train_original.csv", index=False)
    pd.DataFrame(y_test, columns=["engagement"]).to_csv(OUTPUT_DIR / "y_test.csv", index=False)
