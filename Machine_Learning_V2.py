# =============================================================================
# MACHINE LEARNING — Lean Pipeline
# WID3006 ML Group Assignment: "Tying the (Data) Knot"
# =============================================================================
# 6 models, 5-fold CV, RandomizedSearchCV on top 3, auto-sklearn baseline,
# SHAP interpretability, calibrated best model. ~5 min total runtime.
# =============================================================================
...
# Param distributions — broad but fast
param_distributions = {
    "Random Forest": {
...
# Pick best by test accuracy (tuned or base)
all_candidates = []
...
fraction_of_positives, mean_predicted_value = calibration_curve(
    y_test, y_proba_cal[:, 1], n_bins=10, strategy="uniform"
)
...
# =============================================================================
# STEP 5: SHAP INTERPRETABILITY (on unresampled data, unbiased)
# =============================================================================
logger.info("STEP 5: SHAP interpretability...")

# Train a fresh RF for SHAP (tree-explainer is fast and model-agnostic for trees)
...
shap.summary_plot(
    ...
    X_train, show=False, max_display=20,
)
...
shap.summary_plot(
    shap_values[1] if isinstance(shap_values, list) else shap_values,
    X_train, plot_type="bar", show=False, max_display=20,
)
...
# =============================================================================
# STEP 6: FEATURE IMPORTANCE (unbiased, unresampled)
# =============================================================================
...
joblib.dump(selected_unbiased, PREPROCESSED_DIR / "selected_features_unbiased.pkl")
logger.info("  Selected %d / %d features.", len(selected_unbiased), X_train.shape[1])


# =============================================================================
# STEP 7: AUTO-SKLEARN COMPARISON
# =============================================================================
logger.info("STEP 7: Auto-sklearn comparison...")

auto_results = None
try:
    import autosklearn.classification

    logger.info("  Running auto-sklearn (2-min budget)...")
    ...
    auto_f1 = f1_score(y_test, y_pred_auto, average="weighted")
    auto_results = {"accuracy": round(auto_acc, 4), "f1": round(auto_f1, 4)}
    logger.info("  auto-sklearn: Accuracy=%.4f, F1=%.4f", auto_acc, auto_f1)
    joblib.dump(automl, OUTPUT_PATH / "autosklearn_model.pkl")
except ImportError:
    logger.warning("  auto-sklearn not installed. Install with: pip install auto-sklearn")
except Exception as e:
    logger.warning("  auto-sklearn failed: %s", e)


# =============================================================================
# STEP 8: FINAL COMPARISON TABLE
# =============================================================================
logger.info("STEP 8: Building final comparison table...")
...
if auto_results:
    final_results.append({
        ...
        "CV Accuracy (mean)": 0,
        "CV Accuracy (std)": 0,
        "Test Accuracy": auto_results["accuracy"],
        "Test F1 (weighted)": auto_results["f1"],
        "Train Time (s)": 0,
    })
...
final_results.append({
    ...
    "CV Accuracy (mean)": round(float(y_train.mean()), 4),
    "CV Accuracy (std)": 0,
    "Test Accuracy": round(float(y_test.mean()), 4),
    "Test F1 (weighted)": 0,
    "Train Time (s)": 0,
})
...
plt.tight_layout()
plt.savefig(OUTPUT_PATH / "final_comparison_v3.png", dpi=150)
plt.close()
...
# =============================================================================
logger.info("=" * 60)
logger.info("ML Pipeline Complete! Best: %s (calibrated)", best_name)
logger.info("=" * 60)
